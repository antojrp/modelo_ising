...
double n_fold(int s[L][L], double h[5][2], int c[L][L]) {
    double suma=0;
    ...
    for(int j = 0; j < L; j++)
    {
        for (int i = 0; i < L; i++)
        {
            if (c[i][j] == clase_selecionada) // Si la clase coincide 
            {
               ...
               {
                    s[i][j] = -s[i][j]; // Cambiar el espín
                    clase(s, c, i, j); // Calcular la clase de espín
                    int dx[] = {-1, 1, 0, 0};
                    int dy[] = {0, 0, -1, 1};
                    int m,o;
                    for(int d=0; d<4; d++)
...
int main()
{
    ...
    for (int j = 0; j < n; j++) {
        ...
        string direccion_energia = "resultados/n_fold/kawasaki_energia_L_" + to_string(L) + "_T_" + format_double(T) + ".dat";
        ofstream salida_energia(direccion_energia);

        string direccion = "resultados/n_fold/ising_data_L_" + to_string(L) + "_T_" + format_double(T) + ".dat";
        ...
        for (int i = 0; i < M0 * L * L; i++) {
            dt=n_fold(s, h, c); // Actualizar el sistema
